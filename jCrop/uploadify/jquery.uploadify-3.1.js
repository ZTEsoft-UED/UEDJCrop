...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '文件没有被上传:';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('队列中已经或者曾经添加过这个文件，您确定要重新上传么？');
					if (!replaceQueueItem) {
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('队列中已经或者曾经添加过这个文件，您确定要重新上传么？');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
